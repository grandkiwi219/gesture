...
본 스크립트는 크롬 확장프로그램만 압축할 시 입니다.
파이어폭스 애드온을 압축할 시 `npm run zip-ff` 를 입력하셔야 합니다.
...
> ### 문제 보고서
>   - 위험
>       - context menu prevent 전달 방식에서의 치명적인 문제<br>-> context menu 가 모두에게 prevent 상태일 때 단 하나의 prevent 상태가 아닌 새로은 iframe 이 나타날 시 루프되는 현상<br>-> 이로인해 생길 수 있는 문제는 브라우저 성능 저하 혹은 컴퓨터 성능 저하 문제<br>-> 해결법?: 기존의 방식은 전파하는 방식, 하나의 중재자가 모두에게 확정된 답안을 전달하는 방식 사용할 것
>
>   - 공통
>       - context menu 가 뜨는 문제<br>→ 조건: '성능 저하 상태' or '초기 로딩에서 사용시 간헐적'
>       - 펜이 초기에 버벅이는 문제<br>→ 조건: '성능 저하 상태'<br>→ 방안: 계속 대기시킬 수 있게 설정할 수 있는 옵션
>       - 연결되지 않은 탭 메세지 송신 오류
>       - 커맨드 변경 감지시 모든 페이지에서 크롬 스토리지에 접근하는 걸 줄일 것
> 
>   - 파이어폭스
